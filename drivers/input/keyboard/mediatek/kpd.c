...
#include <linux/clk.h>
//zhenglihong 20160511 begin
#ifdef CONFIG_WIND_HALL_SUPPORT
#include <linux/irq.h>
#include <linux/gpio.h>
#include <linux/interrupt.h>
#define GPIO_HALL_1_PIN 2
extern 	unsigned int hall_irq;
#endif
//zhenglihong 20160511 end
...
void  kpd_hall_handle(unsigned long data)
{
	...
	if(cur_hall_state) {
	   ...
	   if (next_hall_state) {
	   	  irq_set_irq_type(hall_irq,IRQF_TRIGGER_RISING);
	   	  printk("ciel high\n");
	   } else {
	   	  irq_set_irq_type(hall_irq,IRQF_TRIGGER_FALLING);
	   	  printk("ciel low\n");
	   }
