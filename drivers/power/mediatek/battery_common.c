...
// hebiao@wing-mobi.com 2016524 begin
#ifdef CONFIG_MTK_CHARGER_RED_DETECTC_OUT_HIGH    
extern int charger_red_output_high(void);
#endif
// hebiao@wing-mobi.com 2016524 end
...
//modified by lvwenkang 20160406 begin
#if defined(AGING_POWER_TEST)
static ssize_t show_Charging_ChargeState(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "g_charger_state = %d\n", g_charger_state);
	return sprintf(buf, "%u\n", g_charger_state);
}

static ssize_t store_Charging_ChargeState(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t size)
{
   unsigned int temp = 0; 
   sscanf(buf, "%u", &temp);
...
static DEVICE_ATTR(Charging_ChargeState, 0664, show_Charging_ChargeState, store_Charging_ChargeState);
#endif
//modified by lvwenkang 20160406 end
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
    ...
    static int counter = 0;		//added by lvwenkang  20160520
	if (BMT_status.UI_SOC <= 0) {
		BMT_status.UI_SOC = 0;
	} else {
	//added by lvwenkang  20160520 b--
...
static void battery_update(struct battery_data *bat_data)
{
	...
	battery_log(BAT_LOG_CRTI, "wind_log_charging_v_0percent_tracking = % d\n", batt_cust_data.v_0percent_tracking);
	...
	battery_log(BAT_LOG_CRTI, "wind_log_only battery_v_0percent_tracking = % d\n", batt_cust_data.v_0percent_tracking);
				...
				const char *node_srting, int *cust_val)
{
	...
	if (of_property_read_u32(np, node_srting, &val) == 0) {
		(*cust_val) = (int)val;
		battery_log(BAT_LOG_CRTI, "Get %s: %d\n", node_srting, (*cust_val));
	} else {
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		//modified by lvwenkang 20160406 begin
		#if defined(AGING_POWER_TEST)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_ChargeState);
		#endif
		//modified by lvwenkang 20160406 end
