...
//liukangping@wind-mobi add sheep begin 20160504
#include <linux/notifier.h>
#include <linux/fb.h>
//liukangping@wind-mobi add sheep end 20160504
...
//liukangping at 20160416 begin
#ifdef CONFIG_WIND_DEVICE_INFO
extern u16 FP_GF318_fwvr;
extern u16 FP_GF318_vendor;
 extern char FP_GF318_id_str[20];
#endif
//liukangping at 20160406 end
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SLEEP:
		...
		mutex_unlock(&gfx1xm_dev->fb_lock);
	    break;
		//liuakangping add sleep begin 20160504
	case GF_IOC_PM_FBCABCK:
		__put_user(gfx1xm_dev->fb_black, (u8 __user *) arg);
		break;
		//liuakangping add sleep end 20160504
	default:
...
static int isUpdate(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	unsigned char* fw316 = NULL; //ranyanhao
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	if(chip_version_config==0x38){		
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
		    ...
		    // liukangping 20160416 begin
			#ifdef CONFIG_WIND_DEVICE_INFO
			FP_GF318_vendor = 0x1;
			gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_CFG_ADDR, &FP_GF318_fwvr);
			#endif
			// liukangping 2016041624 end
			...
			#endif
			// zhaozhensen 20160416 end
			// liukangping 20160416 end
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group0, GFX1XM_CFG_LEN);   //liukangping 20160308
		...
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: %s, read Vendor ID is---liuakngping111 !!!\n", __func__);
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg6_info_group0, GFX1XM_CFG_LEN);
		} else if (!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_2, 1)) {
			gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: %s, read Vendor ID is ---liuakngping222!!!\n", __func__);
		    ...
		    memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg6_info_group0, GFX1XM_CFG_LEN);//liukangping 20160308
			//gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: Vendor ID not matched!!! no send\n");
			//return 0;
		}
	}
...
static int gfx1xm_check_9p_chip(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	if (gfx1xm_dev == NULL)
	{ 
		gfx1xm_debug(DEFAULT_DEBUG,"%s, time_out=%d\n", __func__, time_out);
		return 0;
	}
	do{
...
static int  gfx1xm_probe(struct spi_device *spi)
{
	...
	u8 compare[6]={0x47,0x46,0x78,0x31,0x38,0x4D};
	...
	#if 1
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,gf318m");
	...
	memcpy(version, gfx1xm_dev->buffer + GFX1XM_RDATA_OFFSET, 16);	
		...
		for(ret = 0; ret <6; ret++)
		{
		     if(version[ret]!=compare[ret])
		     	{
			    printk("wind-log_101");
				devm_pinctrl_put(gfx1xm_dev->pctrl);// release reset
			    return -ENODEV;
		     	}
		}
		//liukangping 20160416 end
		temp_strptr =  kzalloc(strlen(saved_command_line) + strlen(" androidboot.fptype=gf318") + 1,
			    GFP_KERNEL);
		strcpy(temp_strptr, saved_command_line);
		strcat(temp_strptr, " androidboot.fptype=gf318");
    ...
    //liukangping 20160416 begin
	sprintf(FP_GF318_id_str, "gf318_%x.%x.%x", version[7], version[8], version[9]);
	//liukangping 20160416 end
...
#error "have't define any product for fw"
	...
	//xielei @20160108
	finger_irq_node= of_find_compatible_node(NULL,NULL, "mediatek,finger_print-eint");
	if(finger_irq_node)
	{
		...
		if(request_irq(gfx1xm_dev->spi->irq,gfx1xm_irq,IRQF_TRIGGER_RISING,"gf318m-eint",NULL)){
	...
	if(chip_version_config==0x38){		
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			g_fp_vendor = 0x1;
			gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_CFG_ADDR, &g_fp_fwvr);
		} else if (!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_2, 1)) {
			g_fp_vendor = 0x2;
			gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_CFG_ADDR, &g_fp_fwvr);
		} else {
...
/*
	switch (version[4]) {
		case '8':
			sprintf(FP_GF516_id_str, "GFx18m_%x.%x.%x", version[7], version[8], version[9]);
			break;
		case '6':
			sprintf(FP_GF516_id_str, "GFx16m_%x.%x.%x", version[7], version[8], version[9]);
			break;
		default:
			FP_GF516_id_str = "unknown";
	}
*/
	#endif
	// liukangping 20160416 end
