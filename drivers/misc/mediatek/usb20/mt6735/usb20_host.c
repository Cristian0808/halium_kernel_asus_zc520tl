...
#include "../../power/mt6735/ncp1854.h"		//tuwenzan add at 20160225 begin
#endif


//yutao add key  h file begin
#ifdef CONFIG_MTK_OTG_HAS_INPUT
#include <linux/input.h>
#endif
//yutao add key  h file end
#ifdef CONFIG_OF
...
#endif
//yutao add usbid key end

//yutao add 20160603 begin
#ifdef CONFIG_MTK_OTG_HAS_INPUT
extern signed int battery_meter_get_charger_voltage(void);
struct task_struct *otg_thread=NULL;
static DEFINE_MUTEX(otg_mutex);
 static int otg_plug_times_flag =0; //yutao add this flag for some boards will cause one plug out irq.
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	//yutao add report otg int key to the input hub begin
	#ifdef CONFIG_MTK_OTG_HAS_INPUT
	...
	#endif
	//yutao add report otg int key to the input hub end
