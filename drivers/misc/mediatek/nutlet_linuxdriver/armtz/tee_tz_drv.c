...
#include "tee_log.h"
#include "rsa.h"
// zhuhy, call tee debug, 20160224
#include <asm/cacheflush.h>



#define CALL_TEE_TRACE		1	// for debug
...
extern 	struct tee_context *tee_context_ext;
...
#ifdef CALL_TEE_TRACE
	dev_dbg(DEV, " [tee_trace] in [%s],[%s %d] try to lock\n", __func__, current->comm, current->pid);
#endif
	mutex_lock(&ptee->mutex);
#ifdef CALL_TEE_TRACE
	dev_dbg(DEV, " [tee_trace] in [%s],[%s %d] get locked\n", __func__, current->comm, current->pid);
#endif
}
...
#ifdef CALL_TEE_TRACE
	dev_dbg(DEV, " [tee_trace] in [%s],[%s %d] unlock\n", __func__, current->comm, current->pid);
#endif
	/*
...
static void handle_rpc_func_cmd_expmod(struct tee_tz *ptee,
						struct teesmc32_arg *arg32)
{
    struct teesmc32_param *params;
    char *get_param;
    ...
    get_param = tee_shm_pool_p2v(DEV, ptee->shm_pool, params->u.memref.buf_ptr);
    param = (TEE_expmod *)get_param;
    printk("%s: qq-----------------------0x%x\n", __func__,param->alen);
    res = ree_expmod(param->a,param->alen, param->b,param->blen,
				param->m,param->mlen, param->r, &(param->rlen));
    printk("%s: qq-----------------------res %d\n", __func__,res);
    if(res == 0)
        arg32->ret = TEEC_SUCCESS; 
    else
...
static void handle_rpc_func_cmd_prime(struct tee_tz *ptee,
						struct teesmc32_arg *arg32)
{
    struct teesmc32_param *params;
    ...
    get_param = tee_shm_pool_p2v(DEV, ptee->shm_pool, params->u.memref.buf_ptr);
    param = (TEE_bufst *)get_param;
    printk("%s: qq-----------------------0x%x\n", __func__,param->rlen);
    res = ree_bn_is_prime(param->r,param->rlen);
	param->is_prime = res;
    printk("%s: qq-----------------------res %d\n", __func__,res);
    if(res >= 0)
        arg32->ret = TEEC_SUCCESS; 
...
static void handle_rpc_func_get_attr(struct tee_tz *ptee,
						struct teesmc32_arg *arg32)
{
	int ret;
	struct teesmc32_param *params;
    ...
    //params[0].u.value.a == addr
    //params[0].u.value.b == size
    //printk("<tcore get memory flag\n");
	ret =  tee_get_shm_flag((void *)(&params[0].u.value.a) , params[0].u.value.b);
	params[0].u.value.a = ret;

	arg32->ret = TEEC_SUCCESS;
	//printk(">tcore get memory flag:%x\n", ret);
	return;
bad:
	arg32->ret = TEEC_ERROR_BAD_PARAMETERS;
}
static void handle_rpc_func_cmd(struct tee_tz *ptee, u32 parg32)
{
    ...
    case NLTEE_RSA_RPC_GET_PQ:
        ...
        break;
		///////////////yr add  20160223
	 case NLTEE_EXPMOD_RPC:
     
		// not implemented yet, yangjun ,20160215
        printk("%s: NLTEE_expmod not implemented yet ,yj\n", __func__);
        handle_rpc_func_cmd_expmod(ptee, arg32);
		//arg32->ret = 0;
        break;

	 case NLTEE_ISPRIME_RPC:
     
		// not implemented yet, yangjun ,20160215
        printk("%s: NLTEE_isprime not implemented yet ,yj\n", __func__);
        handle_rpc_func_cmd_prime(ptee, arg32);
		//arg32->ret = 0;
        break;
	...
	case NLTEE_EVENT_RPC_WAIT_FOR_EVENT:
	{
		...
		//msleep(3000); //michael-li@tcl.com
		arg32->ret = 0;

		break;
	}
    case NLTEE_EVENT_RPC_GET_FLAG:
    {
		
		//printk("NLTEE_EVENT_RPC_GET_FLAG\n");
        handle_rpc_func_get_attr(ptee, arg32);
        break;
    }
...
static u32 handle_rpc(struct tee_tz *ptee, struct smc_param *param)
{
	struct tee_shm *shm;
	int cookie;
    printk("%s,process cmd:0x%lx\n", __func__, (long unsigned int)TEESMC_RETURN_GET_RPC_FUNC(param->a0));
...
#if 0
		wmb();
		__flush_dcache_area(&param, sizeof(param));
		smp_call_function_single(smp_processor_id(),(smp_call_func_t)&tee_smc_call,(void*)&param,0);
		__flush_dcache_area(&param, sizeof(param));
		rmb();
#endif
		ret = param.a0;
 ...
 *	| TEE_SHM_TEMP | TEE_SHM_FROM_RPC)) != 0 ) {
	...
	//printk("%s: kaddr=%p, paddr=%p, shm=%p, size %x:%x,flags:%x\n",
	//	__func__, shm->kaddr, (void *)shm->paddr, shm,
	//	(unsigned int)shm->size_req, (unsigned int)shm->size_alloc,shm->flags);
	shm_temp = (struct tee_shm *)kzalloc(sizeof(struct tee_shm),GFP_KERNEL);
	
    #ifdef SHM_LIST_DEBUG
	printk("get sharmemory flag\n");
    #endif
	memcpy(shm_temp,shm, sizeof(struct tee_shm));
   
    #ifdef SHM_LIST_DEBUG
	printk("after copy kaddr:%p,%p,paddr:0x%lx,0x%lx flag:%x\n",  shm->kaddr, shm_temp->kaddr,(unsigned long)(shm->paddr),(unsigned long)(shm_temp->paddr), shm_temp->flags);
    #endif
	
	// zhuhy@nutlet, add lock for shm,20160221
	
#ifdef SHM_LIST_DEBUG
	dev_dbg(DEV, "[shm_trace] %d try to lock\n", __LINE__);
#endif
	mutex_lock(&tee_context_ext->lock);	
#ifdef SHM_LIST_DEBUG
	dev_dbg(DEV, "[shm_trace] %d locked\n", __LINE__);
#endif	
	tee_context_ext_print();
    list_add_tail(&shm_temp->entry,&tee_context_ext->list_shm);
	tee_context_ext_print();
	
    #ifdef SHM_LIST_DEBUG
	printk(">%s:,addlist flag:%x \n", __func__ , shm->flags);
	#endif
	ret2 = tee_get_shm_flag(shm->kaddr,shm->size_alloc > 10?shm->size_alloc -10:shm->size_alloc);
	
    #ifdef SHM_LIST_DEBUG
	printk("access kaddr:%x,ret=%x\n",*(uint32_t*) shm->kaddr,ret2);
	#endif
	ret2 = -3;
	ret2 = tee_get_shm_flag((void*)(&(shm->paddr)),shm->size_alloc > 10?shm->size_alloc -10:shm->size_alloc);
	
    #ifdef SHM_LIST_DEBUG
	printk("access paddr:%x,ret=%x\n",(int)(shm->paddr),ret2);
	#endif
	
// zhuhy@nutlet, add lock for shm,20160221	
	mutex_unlock(&tee_context_ext->lock);
#ifdef SHM_LIST_DEBUG	
	dev_dbg(DEV, "[shm_trace] %d unlocked\n", __LINE__);
#endif
	return shm;
}
...
static void tz_free(struct tee_shm *shm)
{
	...
	if (!ret) {
		...
		dev_dbg(DEV, "[shm_trace] %d locked\n", __LINE__);
#endif
		ret2 = tee_context_ext_del(shm_temp);		
		// zhuhy@nutlet, add lock for shm,20160221
...
#endif		
		kfree(shm_temp);		
		//printk("< end delet shm list, ret = %d\n", ret2);
		devm_kfree(tee->dev, shm);
		shm = NULL;
	}
}
