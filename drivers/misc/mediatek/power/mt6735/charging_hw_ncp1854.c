...
static unsigned int charging_hw_init(void *data)
{
	...
	ncp1854_set_ieoc(0x4); /* cut off current = 200mA   modified by lvwenkang 20160511*/
...
static unsigned int charging_set_cv_voltage(void *data)
{
	...
	if (batt_cust_data.high_battery_voltage_support)
		cv_value = BATTERY_VOLT_04_380000_V;   //modified by lvwenkang  20160509

	/* use nearest value */
	array_size = GETARRAYNUM(VBAT_CV_VTH);
	set_chr_cv = bmt_find_closest_level(VBAT_CV_VTH, array_size, cv_value);

	battery_log(BAT_LOG_CRTI, "wind_log_set_chr_cv = %d\n",set_chr_cv);//added by lvwenkang  20160509
...
static unsigned int charging_set_current(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "wind_log_set_chr_current = %d\n",set_chr_current);//added by lvwenkang  20160509
